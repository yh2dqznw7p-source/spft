...
public class BindCommand extends Command {
    ...
    private void add(String[] args) {
        ...
        Module module = MaxDLCClient.get().getModuleManager().getModule(args[1]);
        ...
        MaxDLCClient.get().getBindManager().add(module, key);
    ...
    private void remove(String[] args) {
        ...
        if (module == null) { CommandManager.chat("§cМодуль §f" + args[1] + "§c не найден."); return; }
        int key = KeyNames.keyOf(args[2]);
        boolean ok = MaxDLCClient.get().getBindManager().remove(module, key);
    ...
    private void clear(String[] args) {
        ...
        Module module = MaxDLCClient.get().getModuleManager().getModule(args[1]);
        if (module == null) { CommandManager.chat("§cМодуль §f" + args[1] + "§c не найден."); return; }
        int n = MaxDLCClient.get().getBindManager().removeAll(module);
        CommandManager.chat("§aУдалено биндов: §f" + n);
    }

    private void list() {
        var binds = MaxDLCClient.get().getBindManager().getBinds();
